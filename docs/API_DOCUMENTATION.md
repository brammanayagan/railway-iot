...
**QA Test Cases:**
- *Positive:* Register with valid name and phone.
- *Negative:* Register without name.
- *Negative:* Register with an already existing phone number.
...
*(Developer Note: Real SMS integration like Twilio would occur here).*
...
```
*Note: `action` should be "ADD" or "REMOVE".*
...
*All routes in `/api/devices` require the `ADMIN` role.*
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const res = await axios.post('https://your-backend.com/api/auth/refresh-token', {
          token: refreshToken,
        });
